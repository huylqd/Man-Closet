...
import { EnterIcon } from "./IconComment";

const Comment = () => {
    const [radomEvents, setRandomEvents] = useState<any>([]);
    const [comments, setComment] = useState<any>([]);
    const [eventOnClick, setEventOnClick] = useState<any>(null);
        ...
        async function fetchEventData() {
            try {
                ...
                const flattenedEventData = Object.entries(data).reduce(function (
                    ...
                    const {key, value}:any = accumulateValue;

                    return {
                        ...currentValue,
                        ...value.map((val: any) => ({ ...val, idParent: key })),
                    };
                },
                    []);
    ...
    const handleViewMore = (eventSTT: any, eventIdParent: any) => {
        setEventOnClick({ STT: eventSTT, idParent: eventIdParent });
        setShowReplies((prevShowReplies: any) => ({
            ...prevShowReplies,
            [eventSTT]: true,
        }));
    };

    const handleHideReplies = (eventSTT: any) => {
        setEventOnClick({})
        setShowReplies({});
    };
    return (
        <div className='w-[95%] bg-white ml-3 rounded-[20px]'>
            ...
            <div className="h-[30rem] overflow-y-scroll scrollbar-thin scrollbar-thumb-rounded-md scrollbar-thumb-gray-400 scrollbar-track-gray-300">
                {radomEvents.map((content: any, index: any) => (
