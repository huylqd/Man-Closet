...
import { getAllCommentByProductId, postComment } from "@/redux/reducer/comment.reducer";
...
const Comment = ({ productId }: props) => {
    ...
    const userId = comment?.map((item, index) => item.user_id);
    const user = userId
        ?.map((item) => {
            const userById = users.filter((items) => items._id == item);
            return userById;
        })
        .flat()
        .concat();
        ...
        <section className="bg-white dark:bg-gray-900 py-8 lg:py-16 antialiased">
            <div className="max-w-2xl mx-auto px-4">
                ...
                {comment?.map((item, index) => {
                    ...
                    const userComment = user.find((i) => i._id == item.user_id)
