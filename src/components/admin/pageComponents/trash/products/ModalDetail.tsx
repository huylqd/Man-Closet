...
import {  getProductDeletedById } from "@/services/products/products";
import { useParams } from "next/navigation";
import React, { useState, useEffect, useRef } from "react";
// import ModalUpdate from './ModalUpdate';
import { v4 as uuidv4 } from 'uuid';
const ModalDetail = ({  id, onClose }: any) => {
    const productId = id;
    ...
    useEffect(() => {
        getProductDeletedById(productId).then(({ data }: any) => setDetail(data))
    }, [])
        ...
        <div className='fixed inset-0 bg-black bg-opacity-25 backdrop-blur-sm flex justify-center items-center'>
            <div className='flex flex-col justify-between lg:flex-row gap-16 lg:items-center p-12 bg-white'>
                <div className='flex flex-col gap-6 lg:w-2/4'>
                    <img ref={imgRef} alt="" className='w-full h-80 aspect-square object-cover rounded-xl' src={detail[0]?.properties?.[0].imageUrl} />
                    <div className='flex  justify-between h-24'>
                        {detail[0]?.properties?.map((url: any) => {
                            return (
                ...
                <form action="" className='flex flex-col gap-4 lg:w-2/4'>
                    ...
                    <div className="md:flex-1 px-4">
                        <h2 className="text-2xl font-bold text-gray-800 dark:text-white mb-2">{detail[0]?.productName}</h2>
                        ...
                        <div className=" mb-4">
                            <div className="mr-4 pb-3">
                                ...
                                <span className="text-gray-600 dark:text-gray-300">{detail[0]?.price}</span>
                            </div>
                            <div>
                        ...
                        <div className="mb-4">
                            ...
                            <div className="flex items-center mt-2">
                                {detail[0]?.properties?.map((url: any,index:number) => {
                                    return (
                        ...
                        <div className="mb-4">
                            ...
                            <div className="flex items-center mt-2">
                                {detail[0]?.properties?.[0].variants?.map((url: any,index:number) => {
                                    return (
                        ...
                        <div className="mr-4 pb-3">
                            ...
                            <span className="text-gray-600 dark:text-gray-300">{detail[0]?.description}</span>
                        </div>
