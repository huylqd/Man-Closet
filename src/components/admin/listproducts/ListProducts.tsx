...
const ListProducts = () => {
  ...
  useEffect(() => {
    getAll(0).then(({ data }: any) => setProducts(data));
  }, []);
  const onhandleRemove = (id: any) => {
    if (confirm("Are you sure you want to remove")) {
      deletePro(id).then(({ data }: any) => {
        // alert("Xóa thành công!");
        setProducts(products.filter((item: any) => item._id !== data._id));
      });
      alert("Xóa thành công!");
    }
    ...
    <div>
      <div className="overflow-x-auto">
        <button
          ...
          className=" mb-4 text-white inline-flex items-center bg-sky-700 hover:bg-sky-800 focus:ring-4 focus:outline-none focus:ring-sky-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800"
        >
          <svg
            ...
            {products.map((data: IProduct) => {
              return (
                ...
                >
                  ...
                  <td
                    scope="row"
                    className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-gray-900"
                  >
      ...
      {showPortal && <Modal isOpen={showPortal} handleClose={handleClosePortal}>
      ...
      />
      <ModalUpdate
        isvisibleUpdate={showModalUpdate}
        update={onhandleUpdate}
        products={product}
        onClosePro={() => setshowModalUpdate(false)}
      />
    </div>
  );
};
